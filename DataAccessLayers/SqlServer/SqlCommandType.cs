...
namespace DataAccessLayer.SqlServer
{
    internal class SqlCommandType
    {
        ...
        internal static ConcurrentDictionary<string, SortedList<string, CommandType>> cacheData;
        ...
        {
            if (cacheData == null)
                cacheData = new ConcurrentDictionary<string, SortedList<string, CommandType>>();
        ...
        private void ExtractSchemaDetail(string connectionString, SqlConnection connection, string collectionName, string columnName, CommandType type)
        {
            ...
            if (!cacheData.ContainsKey(connectionString))
            {
                cacheData.TryAdd(connectionString, dbObjects);
            }
